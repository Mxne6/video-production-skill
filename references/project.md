...
- `assets`: `[{id,status,path,source,purpose,prompt,ratio,composition,factual_constraints}]`。status 为 ready 或 waiting_external；场景 assets 引用这些 ID。
...
`visual_plan: {medium,reason}` 记录媒介选择与表达理由，配音前预览工具据此检查素材引用；`estimated_duration` 为正数秒数，包含该幕停顿与尾部留白，配音前按帧取整用于预算检查，实测音轨到位后替代估时。图片资产额外记录 `sha256`，HTML 图片用 `data-asset-id` 绑定素材 ID，详见 [视觉规划](visual-planning.md)。

`visual_contract`（可选）记录视频视觉适配字段：`style_mode`（editorial / swiss）、`theme`、`ratio`（3:4、16:9、9:16 或 4:3）、`recipe`、`focal_point`、`subtitle_safe_area` 和 `motion`。新视觉适配场景应使用 `references/video-scene-contract.json` 的字段约定，并在静态设计审核通过后再加入轻动效。历史项目缺少这些字段时不自动迁移；修改或续作时按当前规则补齐。

`design_preview_manifest` 与 `user_design_review`（可选）绑定逐页 PNG 预览清单和用户明确反馈。存在预览清单时，用户批准前不得进入 `approved_for_motion` 或正式视频导出；技术校验、主 Agent 查看或 worker 报告不能代替用户批准。
...
`studio init` 的 generic 默认仍是 1920×1080；新义合成使用 `--preset xyzchem` 的 1080×1440。现有项目不会自动改变画幅或补填未知时长。固定片尾只能通过显式 `attach_brand_outro.py` 装配，role 本身不触发导入。旧导出文件保留；重新生产时缺当前设计审核需按实际静态查看记录补齐，不自动伪造通过。

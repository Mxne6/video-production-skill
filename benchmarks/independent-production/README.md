...
此目录不参与日常视频生产、自动测试发现或正式门禁。只有用户明确要求比较目标模型、推理档位或独立完成率时才使用。下列资料原样保留，避免默认生产流程被基准治理占用。
